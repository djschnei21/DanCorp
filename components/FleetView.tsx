...
import { READINESS_LABEL, STATUS_LABEL } from "@/lib/format";
...
export function FleetView() {
  const vehicles = getFleet(new Date(useShift()));
            ...
            <p className="border-t border-card-04 px-5 py-3 text-sm">
              {vehicle.nextMissionId ? (
                <>
                  ...
                  {vehicle.nextStatus ? (
                    <span className={vehicle.nextStatus === "in_flight" ? "text-accent" : "text-muted"}>
                      {` · ${STATUS_LABEL[vehicle.nextStatus]}`}
                    </span>
                  ) : null}
                </>
              ) : (
